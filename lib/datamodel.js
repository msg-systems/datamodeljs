...
(function (root, factory) {
    ...
    _em.dm.sanityCheckFieldType = function (methodId, cls, field, fieldType, fieldValue) {
        if (typeof fieldValue != fieldType && fieldValue !== null)
    ...
    _em.dm.import = function(method, dm, cls, obj, example, newObjs) {
        var objPrimaryKeyValue = _em.dm.getPrimaryKeyValueFromObject(dm, cls, obj);
        if (objPrimaryKeyValue === undefined)
            throw _em.exception(method, "Can not find an object that matches the given obj. Try using findByExample yourself before calling import.");
        var possibleObj = dm.findById(cls, objPrimaryKeyValue);
        if (possibleObj === undefined)
            throw _em.exception(method, "Can not find an object that matches the given obj. Try using findByExample yourself before calling import.");
        if (possibleObj._isStub)
            throw _em.exception(method, "Given obj is a stub object. Try creating the obj first before you import value in it.");

        var examplePrimaryKeyValue = _em.dm.getPrimaryKeyValueFromObject(dm, cls, example);
        var primaryKeyField = _em.dm.getPrimaryKeyNameForClass(dm, cls);
        if (examplePrimaryKeyValue !== undefined)
            _em.dm.sanityCheckFieldType(method, cls, primaryKeyField, dm.entities[cls][primaryKeyField].type, examplePrimaryKeyValue);
        ...
        if (examplePrimaryKeyValue === undefined || objPrimaryKeyValue === examplePrimaryKeyValue) {
            ...
            try {
                                ...
                                default:
                                    ...
                                    if (Object.prototype.toString.call(singleValue) === Object.prototype.toString.call({})) {
                                        ...
                                        if (relPrimaryKeyValue === "" && relPrimaryKeyValue === undefined) {
                                            // create a object --> it will be transient
                                            relObj = _em.dm.findOrCreateObjectByPayload(method, dm, specField, singleValue, newObjs);
                                        } else {
                                            relObj = dm.findById(specField.type, relPrimaryKeyValue);
                                            if (relObj === undefined) {
                                                // if not find create object
                                                relObj = _em.dm.findOrCreateObjectByPayload(method, dm, specField, singleValue, newObjs);
                                            } else {
                                                // if find import it
                                                relObj = _em.dm.import(method, dm, specField.type, relObj, singleValue, newObjs);
                                            }
                                        }
                                    } else {
                                        relPrimaryKeyValue = singleValue;
                                        relObj = dm.findById(specField.type, relPrimaryKeyValue);
                                        // if relObj was found nothing to do here, because no data is given to import
                                        if (relObj === undefined)
                                            relObj = _em.dm.findOrCreateStubObjectByKey(dm, specField, singleValue, newObjs);
                                    }
                        ...
                        switch (specField.type) {
                            ...
                            default:
                                ...
                                if (Object.prototype.toString.call(payloadValue) === Object.prototype.toString.call({})) {
                                    relPrimaryKeyValue = payloadValue[_em.dm.getPrimaryKeyNameForClass(dm, specField.type)];
                                    if (relPrimaryKeyValue === "" && relPrimaryKeyValue === undefined) {
                                        // create a object --> it will be transient
                                        ...
                                        if (relObj === undefined) {
                                            ...
                                            relObj = _em.dm.import(method, dm, specField.type, relObj, payloadValue, newObjs);
                                        }
                                    }
                                } else {
                ...
                // all stub objects created newly must be destroyed as well since at least one creation did not work
                _em.dm.destroyAllNewObjs(dm, newObjs);
                throw e;
            }
        } else {
            throw _em.exception(method, "Can not import object with primaryKeyValue(" + examplePrimaryKeyValue +
    ...
    _em.dm.create = function(method, dm, cls, payload, newObjs) {
        ...
        if (objIsTransient) {
            ...
            var uuid = _em.dm.UUIDforClass(dm, cls);
            var relPrimaryKeyType = dm.entities[cls][primaryKeyField].type;
            if (relPrimaryKeyType === "number")
                uuid = +uuid;
            obj[primaryKeyField] = uuid;
            // push the object into the collections and initially set its flags
            dm.entityObjs[cls].idx[uuid] = obj;
            dm.entityObjs[cls].objs.push(obj);
        } else {
    ...
    _em.dm.UUIDforClass = function (dm, cls) {
        var random = Math.round(Math.random() * 1000000);
        var currentTime = new Date().getTime();
        var uuid =  random + "" + currentTime;
        if (dm.entityObjs[cls].idx[uuid]){
            return _em.dm.UUIDforClass(cls);
        } else {
            return uuid;
        }
    };
    ...
    _em.dm_api.prototype = {
        ...
        findAllTransient : function (cls) {
            ...
            _em.dm.sanityCheckClassArgument(methodId, this, cls, false);
            var result = [];
            for (var key in this.entityObjs[cls].objs){
                if (!this.entityObjs[cls].objs.hasOwnProperty(key))
                    continue;
                var obj = this.entityObjs[cls].objs[key];
                if (obj._isTransient)
                    result.push(obj)
            }
            return result;
        ...
        import : function (cls, obj, example) {
            ...
            _em.dm.sanityCheckClassArgument(methodId, this, cls, false);
            return _em.dm.import(methodId, this, cls, obj, example, {});
        },
